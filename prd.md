...
[
  {
    ...
    "steps": [
      ...
      "Verify types: import { HashRouter } from 'react-router-dom' should resolve. If not: bun add -d @types/react-router-dom",
      "Update src/main.tsx to wrap App in HashRouter",
      "Update src/App.tsx to use Routes/Route for / and /trophies",
      "Create placeholder src/views/TrophyView.tsx",
      "Create public/404.html with script redirect to index.html (GitHub Pages SPA trick)",
      "Add React error boundary wrapping each Route",
  ...
  },
  {
    ...
    "category": "setup",
    "description": "Extract collectibles tracker into its own view component",
    "steps": [
      "Create src/views/ and src/components/ directories",
      "Create src/views/CollectiblesView.tsx — move all collectibles logic from App.tsx",
      "Create src/views/CollectiblesView.css — move collectibles styles from App.css",
      "Keep App.tsx as router shell only. Use relative imports (no @/ aliases)",
  ...
  },
  {
    ...
    "category": "setup",
    "description": "Create typed trophy data module from JSON source. See prd-specs.md for data model",
    "steps": [
      "Move TLOU_REMASTERED_TROPHIES.json into src/ (within tsconfig include scope)",
      "Add 'resolveJsonModule': true to tsconfig.app.json",
      "Create src/trophyData.ts with interfaces: Trophy, TrophySection, RoadmapStage, TrophyType",
      "JSON uses snake_case, TypeScript uses camelCase — write mapping function to transform at import",
      "JSON has different structures (base_game_trophies flat array vs dlc objects). Normalize to uniform TrophySection[]",
